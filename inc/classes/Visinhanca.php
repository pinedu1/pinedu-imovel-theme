...
class Visinhanca extends Pinedu_Base implements PineduPostType {
  ...
  public function query( ) {
    ...
    if ( get_post_meta( $this->post->ID, 'ativarVenda', true ) == '1' ) {
      $meta_query[] = [ 'key' => 'ativarVenda', 'value' => '1', 'compare' => '=' ];
    }
    if ( get_post_meta( $this->post->ID, 'ativarLocacao', true ) == '1' ) {
      $meta_query[] = [ 'key' => 'ativarLocacao', 'value' => '1', 'compare' => '=' ];
    }
    if ( get_post_meta( $this->post->ID, 'ativarLancamento', true ) == '1' ) {
      $meta_query[] = [ 'key' => 'ativarLancamento', 'value' => '1', 'compare' => '=' ];
    }
    $meta_query[] = [ 'key' => 'regiaoCorretagem', 'value' => get_post_meta( $this->post->ID, 'regiaoCorretagem', true ), 'compare' => '=' ];
