...
$tipo_imovel = get_query_var( 'tipoImovel', '' );
$gpb = new Gpb( );
$data = $gpb->localizacao( $tipo_imovel );
$tipo_imovel = false;
    ...
    <select id="estado" name="estado" onchange="estadoChange( this );">
      ...
      <?php if ( $data->estados && ! empty( $data->estados ) ): ?>
        <?php foreach ( (array) $data->estados as $uf ): ?>
          <option value="<?php echo esc_attr( $uf->id ); ?>" <?php echo ( true == $uf->selected ) ?? 'selected' ?>><?php echo esc_html( $uf->uf ); ?></option>
        <?php endforeach; ?>
      <?php endif; ?>
    </select>
  </div>
    ...
    <select id="cidade" name="cidade" onchange="cidadeChange( this );">
      ...
      <?php if ( $data->cidades && ! empty( $data->cidades ) ): ?>
        <?php foreach ( (array) $data->cidades as $cidade ): ?>
          <option value="<?php echo esc_attr( $cidade->id ); ?>" <?php echo ( true == $cidade->selected ) ?? 'selected' ?>><?php echo esc_html( $cidade->nome ); ?></option>
        <?php endforeach; ?>
      <?php endif; ?>
    </select>
    ...
    <select id="bairro" name="bairro">
      ...
      <?php if ( $data->bairros && ! empty( $data->bairros ) ): ?>
        <?php foreach ( (array) $data->bairros as $bairro ): ?>
          <option value="<?php echo esc_attr( $bairro->id ); ?>" <?php echo ( true == $bairro->selected ) ?? 'selected' ?>><?php echo esc_html( $bairro->nome ); ?></option>
        <?php endforeach; ?>
      <?php endif; ?>
    </select>
    <div class="loading-overlay"></div>
...
<div class="row">
  ...
  <div class="col col-apt">
    <label for="apt"><?php echo $tipo_imovel ?? $tipo_imovel->campoUnidade; ?></label>
    <input type="text" class="form-control" id="apt" name="apt" placeholder="<?php echo $tipo_imovel ?? $tipo_imovel->campoUnidade; ?>" value="<?php echo $apt; ?>">
  </div>
  <div class="col col-blc">
    <label for="blc"><?php echo $tipo_imovel ?? $tipo_imovel->campoBloco; ?></label>
    <input type="text" class="form-control" id="blc" name="blc" placeholder="<?php echo $tipo_imovel ?? $tipo_imovel->campoBloco; ?>" value="<?php echo $blc; ?>">
  </div>
