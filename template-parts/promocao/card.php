...
<section class="property-card promocao" itemscope itemtype="http://schema.org/RealEstateListing">
  ...
  <table class="caracteristicas" role="table" aria-label="Informações básicas do imóvel">
    <tbody>
      <tr>
      <?php if ( true ): ?>
        <?php if( $post->ARUTIL ): ?><td itemprop="floorSize" title="<?php echo  $post->ARUTILNome; ?>"><?php echo formata_valor( $post->ARUTIL, 2); ?><br><i class="<?php echo  $icons[ 'ARUTIL' ]; ?>"></i></td><?php endif;?>
        <?php if( $post->DOR ): ?><td itemprop="numberOfRooms" title="<?php echo  $post->DORNome; ?>"><?php echo $post->DOR; ?><br><i class="<?php echo $icons[ 'DOR' ]; ?>"></i></td><?php endif;?>
        <?php if( $post->SUI ): ?><td itemprop="numberOfRooms" title="<?php echo  $post->SUINome; ?>"><?php echo $post->SUI; ?><br><i class="<?php echo $icons[ 'SUI' ]; ?>"></i></td><?php endif;?>
        <?php if( $post->BAN ): ?><td itemprop="numberOfBathroomsTotal" title="<?php echo  $post->BANNome; ?>"><?php echo $post->BAN; ?><br><i class="<?php echo $icons[ 'BAN' ]; ?>"></i></td><?php endif;?>
        <?php if( $post->GAR ): ?><td itemprop="parkingSpace" title="<?php echo  $post->GARNome; ?>"><?php echo $post->GAR; ?><br><i class="<?php echo $icons[ 'GAR' ]; ?>"></i></td><?php endif;?>
      <?php else: ?>
        <?php if( $post->ARUTIL ): $txt = $post->ARUTIL. " " . Promocoes::corta_texto( $post->ARUTILNome, 5); ?><td itemprop="floorSize"><?php echo $txt; ?></td><?php endif;?>
        <?php if( $post->DOR ): $txt = $post->DORPromocoes::corta_texto( $post->DORNome, 5 ); ?><td itemprop="numberOfRooms"><?php echo $txt ?></td><?php endif;?>
        <?php if( $post->BAN ): $txt = $post->BANPromocoes::corta_texto( $post->BANNome, 5); ?><td itemprop="numberOfBathroomsTotal"><?php echo $txt; ?></td><?php endif;?>
        <?php if( $post->GAR ): $txt = $post->GARPromocoes::corta_texto( $post->GARNome, 5 ); ?><td itemprop="parkingSpace"><?php echo $txt; ?></td><?php endif;?>
      <?php endif; ?>
      </tr>
    </tbody>
  </table>
